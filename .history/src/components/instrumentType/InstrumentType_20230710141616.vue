  ...
  <div class="l-tab-content">
    <el-row>
      <el-col :span="24" style="height: 40px;">
        ...
        <el-input v-model="search" placeholder="仪器类型/代号" clearable @input="inputSearch(search)"
    ...
    <el-table :data="InstrumentTable" border style="width: 100%; flex: 1;" size="small" @row-click="rowClick"
      ...
      <el-table-column label="操作" fixed="right" width="120px" align="center">
        <template #default="scope">
          ...
          <el-popconfirm title="确定删除这条数据吗？" @confirm="delEvent(scope.row, false)" confirm-button-text="确认"
...
const delEvent = (row, sign) => {
  ...
  function del(ids) {
    axios.delete(measureModule + "toolType/", {
      data: {
        ids: ids
      }
    }).then(res => {
