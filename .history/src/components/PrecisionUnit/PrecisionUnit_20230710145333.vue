<template>
  <div class="l-tab-content">
    <el-row>
      <el-col :span="24" style="height: 40px;">
        <el-button type="primary" @click="customerData.isShowForm = true, customerData.addOrMod = false">新增</el-button>
        <el-button type="danger" @click="delEvent({}, false)">删除</el-button>
        <el-input v-model="search" placeholder="精度单位/代号" clearable @input="inputSearch(search)"
          style="width: 200px;position:absolute;right: 0;">
          <template #prefix>
            <el-icon class="el-input__icon">
              <Search />
            </el-icon>
          </template></el-input>
      </el-col>
    </el-row>
    <el-table :data="InstrumentTable" border style="width: 100%; flex: 1;" size="small" @row-click="rowClick"
      ref="multipleTableRef" :highlight-current-row="true">
      <el-table-column type="selection" align="center" />
      <el-table-column type="index" label="序号" width="55px" align="center" />
      <el-table-column prop="unit_type_name" label="单位类型" align="center" />
      <el-table-column prop="name" label="精度单位" align="center" />
      <el-table-column prop="code" label="代号" align="center" />
      <el-table-column label="操作" fixed="right" width="120px" align="center">
        <template #default="scope">
          <el-button type="default" size="small" @click="rowProduct(scope.row)"><el-icon>
              <EditPen />
            </el-icon></el-button>
          <el-popconfirm title="确定删除这条数据吗？" @confirm="delEvent(scope.row,true)" confirm-button-text="确认"
            cancel-button-text="取消">
            <template #reference>
              <el-button type="danger" size="small"><el-icon>
                  <DeleteFilled />
                </el-icon>
              </el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination background layout="sizes, prev, pager, next,jumper" :total="customerData.count_1" small
      :page-sizes="[10, 20, 50, 100, 1000]" v-model:current-page="page" v-model:page-size="limit" />
  </div>
  <el-dialog v-model="customerData.isShowForm" :title="customerData.addOrMod ? '修改' : '新增'" width="20%" destroy-on-close>
    <AddOrmod v-model:change="customerData.addOrMod" v-model:rowData="rowData" @reloadTable="getInstrument" />

  </el-dialog>
</template>

<script lang="ts" setup>
//模块导入
import { ref, computed, watch, } from "vue"
import { ElMessage, ElLoading, ElTable, ElMessageBox } from 'element-plus';
import axios from 'axios';
import AddOrmod from './addOrmod.vue'
import useUserData from '../../store/User.js'
import userCustomer from '../../store/store.js'
import { Loading } from "element-plus/es/components/loading/src/service";
import { fa } from "element-plus/es/locale";
import { store } from "@ice/stark-data"

//状态声明
const customerData = userCustomer()
const InstrumentTable = ref([])
const count = ref(0)
const page = ref(1)
const limit = ref(20)
const rowData = ref("")
const search = ref("")
const loading = ref(false)
const multipleTableRef = ref<InstanceType<typeof ElTable>>()
let timeout = null
const { measureModule } = store.get("APIs")["1.1.1"]


//单击行选中
const rowClick = (row) => {
  multipleTableRef.value?.toggleRowSelection(row, undefined)
}

//搜索
const inputSearch = (value) => {
  clearTimeout(timeout);
  timeout = setTimeout(() => {
    getInstrument({ search: value })
  }, 1000);
}

//删除
const delEvent = (row, sign) => {
  if (!sign) {
    let rowData = multipleTableRef.value!.getSelectionRows()
    if (!rowData.length) return ElMessage.warning("未勾选数据！")
    ElMessageBox.confirm(
      `确认删除这${rowData.length}条数据吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }

    ).then(() => del(rowData.map(v => v.id))).catch(() => { })

  } else del([row.id])
  function del(ids) {
    axios.delete(measureModule + "precisionUnit/", {
      data: {
        ids: ids
      }
    }).then(res => {
      let { data } = res
      if (data.code !== 200) return ElMessage.warning(data.msg)
      ElMessage.success(data.msg)
      getInstrument()
    })
  }
}
//修改
const rowProduct = (row) => {
  rowData.value = row
  customerData.isShowForm = true
  customerData.addOrMod = true
}

const getInstrument = (search = {}) => {
  loading.value = true
  axios.get(measureModule + "precisionUnit/", {
    params: {
      page: page.value,
      limit: limit.value,
      ...search
    }
  }).then(res => {
    let { data } = res
    if (data.code !== 200) return ElMessage.warning(data.msg)
    InstrumentTable.value = res.data.data
    customerData.count_1 = res.data.count
    loading.value = false

  }).catch(err => {
    ElMessage.error(err.code);
    loading.value = false
  })
}

// 使用 watch 侦听多个状态的变化
watch([page, limit], () => {
  getInstrument()
}, { immediate: true });


</script>

<style scoped></style>