  ...
  <div class="l-tab-content">
    <el-row>
      <el-col :span="24" style="height: 40px;">
        ...
        <el-input v-model="search" placeholder="仪器规格/代号" clearable @input="inputSearch(search)"
    ...
    <el-table :data="InstrumentTable" border style="width: 100%; flex: 1;" size="small" @row-click="rowClick"
      ...
      <el-table-column prop="specs" label="仪器仪表规格" align="center" />
...
const delEvent = (row, sign) => {
  ...
  function del(ids) {
    axios.delete(measureModule + "toolSpecs/", {
      data: {
        ids: ids
      }
    }).then(res => {
