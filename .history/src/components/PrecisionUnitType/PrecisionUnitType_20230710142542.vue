<template>
    <div class="l-tab-content">
      <el-row>
        <el-col :span="24" style="height: 40px;">
          ...
          <el-input v-model="search" placeholder="精度单位类型/代号" clearable @input="inputSearch(search)"
      ...
      <el-table :data="InstrumentTable" border style="width: 100%; flex: 1;" size="small" @row-click="rowClick"
        ...
        <el-table-column prop="name" label="精度单位类型" align="center" />
  ...
  function del(ids) {
    axios.delete(measureModule + "precisionUnitType/", {
      data: {
        ids: ids
      }
    }).then(res => {
  ...
  const getInstrument = (search = {}) => {
    loading.value = true
    axios.get(measureModule + "precisionUnitType/", {
      params: {
        page: page.value,
        limit: limit.value,
        ...search
