<template>
    <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" class="demo-ruleForm" :size="formSize" label-width="100px" status-icon>
        <el-form-item label="精度单位类型" prop="name">
    ...
    await formEl.validate((valid, fields) => {
        if (valid) {
            axios({
                ...
                url: measureModule + "precisionUnitType/",
                data: ruleForm
            }).then(res => {
                let { data } = res
