<template>
    <div class="l-tab-content">
      ...
      <el-table :data="InstrumentTable" border style="width: 100%; flex: 1;" size="small" @row-click="rowClick"
        ...
        <el-table-column prop="typeName" label="仪器仪表名称" align="center" />
        <el-table-column prop="code" label="名称代号" align="center" />
  ...
  const confirmEvent = (row) => {
    axios.delete(measureModuleUrl + "toolName/", {
      data: {
        id: row.id
      }
    }).then(res => {
  ...
  const getInstrument = (search = {}) => {
    loading.value = true
    axios.get(measureModuleUrl + "toolName/", {
      params: {
        page: page.value,
        limit: limit.value,
        ...search
