<template>
  <div class="l-tab-content">
    <el-row>
      <el-col :span="24" style="height: 40px;">
        ...
        <!-- <el-button>修改</el-button> -->
          <el-button type="danger" @click="delEvent({}, false)">删除</el-button>
        <el-input v-model="search" placeholder="仪器名称/代号" clearable @input="inputSearch(search)"
    ...
    <el-table :data="InstrumentTable" border style="width: 100%; flex: 1;" size="small" @row-click="rowClick"
      ...
      <el-table-column prop="code" label="仪器仪表编码" align="center" />
      <el-table-column prop="toolName" label="仪器名称" align="center" />
      <el-table-column prop="toolType" label="仪器类型" align="center" />
      <el-table-column prop="toolspecs" label="仪器规格" align="center" />
      <el-table-column prop="controlPoint" label="控制要点" align="center" />
      <el-table-column prop="precisionScale" label="最小分度值" align="center" />
      <el-table-column prop="precisionUnit" label="精度单位" align="center" />
      <el-table-column prop="code" label="管理类别" align="center">
        <template #default="scope">
        <span v-if="scope.row.toolCategory === 1">A</span>
        <span v-else-if="scope.row.toolCategory === 2">B</span>
        <span v-else>-</span>
      </template>

      </el-table-column>>
      <el-table-column prop="code" label="校准类别" align="center">
        <template #default="scope">
        <span v-if="scope.row.manageType === 1">外部校准</span>
        <span v-else-if="scope.row.manageType === 2">内部校准</span>
        <span v-else>-</span>
      </template>
      ...
      <el-table-column label="操作" fixed="right" width="120px" align="center">
        <template #default="scope">
          ...
          <el-popconfirm title="确定删除这条数据吗？" @confirm="delEvent(scope.row, true)" confirm-button-text="确认"
  ...
  <el-dialog v-model="customerData.isShowForm" :title="customerData.addOrMod ? '修改' : '新增'" destroy-on-close>
...
//删除
const delEvent = (row, sign) => {
  if (!sign) {
    let rowData = multipleTableRef.value!.getSelectionRows()
    if (!rowData.length) return ElMessage.warning("未勾选数据！")
    ElMessageBox.confirm(
      `确认删除这${rowData.length}条数据吗？`,
      '提示',
      {
    ...
    ).then(() => del(rowData.map(v => v.id))).catch(() => { })

  } else del([row.id])
  function del(ids) {
    axios.delete(measureModule + "measureTool/", {
      data: {
        ids: ids
      }
    }).then(res => {
...
const getInstrument = (search = {}) => {
  loading.value = true
  axios.get(measureModule + "measureTool/", {
    params: {
      page: page.value,
      limit: limit.value,
      ...search
