    ...
    <el-col :span="12">
        <el-form-item label="仪器规格" prop="toolspecs">
            <el-select v-model="ruleForm.toolspecs" placeholder="仪器规格" filterable collapse-tags >
                <el-option v-for="item in toolspecs" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
...
//获取select数据
onMounted(async () => {
    axios.get(measureModule+"toolType/",{   params: {
            page: 1,
            limit: 10000,
        }}).then(res => {
        let { data } = res
    ...
    axios.get(measureModule+"toolName/",{   params: {
            page: 1,
            limit: 10000,
        }}).then(res => {
        let { data } = res
    ...
    axios.get(measureModule+"toolSpecs/",{   params: {
            page: 1,
            limit: 10000,
        }}).then(res => {
        let { data } = res
