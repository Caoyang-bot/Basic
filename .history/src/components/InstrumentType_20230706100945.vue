<template>
  <div class="l-tab-content">
    ...
    <el-table :data="InstrumentTable" border style="width: 100%;" size="small" @row-click="rowClick" ref="multipleTableRef"
...
const confirmEvent = (row) => {
  axios.delete("http://192.168.1.115:8119/measure/toolType/", {
    data: {
      id: row.id
    }
  }).then(res => {
...
function getInstrument() {
  loading.value = true
  axios.get("http://192.168.1.115:8119/publicModule/InstrumentName/", {
    params: {
      page: page.value,
      limit: limit.value,
    }
